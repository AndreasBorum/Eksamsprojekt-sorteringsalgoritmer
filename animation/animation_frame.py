...
class AnimationFrame(tk.Frame):
    def __init__(self, parent, sorting_type):
        ...
        self.canvas.grid(row=0, column=0, pady=10,
                         columnspan=2, padx=10, sticky="nsew")

        self.start_frame = ctk.CTkFrame(self)
        self.start_frame.grid(row=1, column=0, pady=10, padx=10, sticky="snew")

        self.start_btn = ctk.CTkButton(
            self.start_frame, text="Start", command=self.start_stop_animation)
        self.start_btn.grid(row=0, column=0, pady=10, padx=10, sticky="nsew")

        column_slider_label = ctk.CTkLabel(
            self.start_frame, text="Number of columns:")
        column_slider_label.grid(row=1, column=0, padx=10, pady=(10, 0))
        self.start_slider = ctk.CTkSlider(
            self.start_frame, from_=4, to=30, command=self.update_canavs_start_culumns)
        self.start_slider.grid(row=2, column=0, pady=(
        ...
        self.play_pause_btn = ctk.CTkButton(
            self.play_frame, text="Play", command=self.play_pause_animation)
        self.play_pause_btn.grid(
            row=0, column=0, pady=10, padx=10, sticky="nsew")

        self.reset_btn = ctk.CTkButton(
            self.play_frame, text="Reset", command=self.reset)
        self.reset_btn.grid(row=0, column=1, pady=10, padx=10, sticky="e")

        speed_slider_label = ctk.CTkLabel(
            self.play_frame, text="Animation speed:")
        speed_slider_label.grid(row=1, column=0, padx=10, pady=(10, 0))
        self.animation_speed_slider = ctk.CTkSlider(
            self.play_frame, from_=35, to=0, number_of_steps=100, command=self.animation_speed)
        self.animation_speed_slider.grid(row=2, column=0, pady=(
            0, 10), columnspan=2, padx=10, sticky="nsew")

        self.animation_step_back_btn = ctk.CTkButton(
            self.play_frame, text="<-- Step", command=self.animation_step_back)
        self.animation_step_back_btn.grid(
            row=3, column=0, pady=10, padx=10, sticky="nsew")

        self.animation_step_forward_btn = ctk.CTkButton(
            self.play_frame, text="Step -->", command=self.animation_step_forward)
        self.animation_step_forward_btn.grid(
            row=3, column=1, pady=10, padx=10, sticky="nsew")
