...
class CanvasAnimation(tk.Frame, CanvasLogic):
    ...
    def draw_columns_defore_start(self, columns):
        ...
        for i in range(1, self.column_amount+1):
            ...
            self.canvas.create_rectangle(pos_x(i), self.pos_y, pos_x(
                i)+self.coll_width, hight(i), fill='gray75')
            self.canvas.create_text(
                pos_x(i)+self.coll_width/2, self.pos_y-10, text=i)
    ...
    def draw_columns_after_start(self, data):
        ...
        for i, j in enumerate(data):
            column_id = self.canvas.create_rectangle(self.pos_x(i), self.pos_y, self.pos_x(
                i)+self.coll_width, self.hight(i), fill='gray75', tags=('tag'+str(j)))
            self.canvas.create_text(self.pos_x(
                i)+self.coll_width/2, self.pos_y-10, text=j, tags=('tag'+str(j)))
    ...
    def swap(self, i1, i2):
        ...
        self.canvas.abs_move(
            self.columns[i1][1], self.pos_x(i2), self.hight(i1))
        self.canvas.abs_move(
            self.columns[i2][1], self.pos_x(i1), self.hight(i2))
