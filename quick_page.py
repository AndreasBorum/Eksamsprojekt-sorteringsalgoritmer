...
class QuickSortPage(AlgorithmPage):
    def __init__(self, app):
        ...
        self.description_label = CTkWrappingLabel(
            self.description_frame, text=import_text.quick_page_text, text_color="black", justify=tk.LEFT)
        self.description_label.grid(
            row=1, column=0, pady=10, padx=10, sticky="ew")

        yt_btn = ctk.CTkButton(self.description_frame, text="Watch Video", command=lambda: webbrowser.open(
            "https://youtu.be/Vtckgz38QHs?t=10", new=1))
        yt_btn.grid(row=2, column=0, pady=10, padx=10)

        flowchart_image = ctk.CTkImage(Image.open(
            r"images\Quick_sort_algorithm.png"), size=(400, 300))
