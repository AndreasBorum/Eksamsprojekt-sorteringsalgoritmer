...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.left_side_panel = ctk.CTkFrame(
            self.main_container, width=170, corner_radius=10)
        self.left_side_panel.pack(
            side=tk.LEFT, fill=tk.Y, expand=False, padx=5, pady=5)
        ...
        self.logo_label = ctk.CTkLabel(
            self.left_side_panel, text="Welcome! \n", font=ctk.CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.scaling_label = ctk.CTkLabel(
            self.left_side_panel, text="UI Scaling:", anchor="w")
        ...
        self.bt_Quit = ctk.CTkButton(self.left_side_panel, text="Quit",
                                     fg_color='#EA0000', hover_color='#B20000', command=self.close_window)
        self.bt_Quit.grid(row=9, column=0, padx=20, pady=10)

        # button to select correct frame IN self.left_side_panel WIDGET
        self.bt_home = ctk.CTkButton(
            self.left_side_panel, text="Home page", command=lambda: self.show_frame(HomePage))
        self.bt_home.grid(row=1, column=0, padx=20, pady=10)

        self.bt_intro = ctk.CTkButton(
            self.left_side_panel, text="Intro page", command=lambda: self.show_frame(IntroPage))
        self.bt_intro.grid(row=2, column=0, padx=20, pady=10)

        self.bt_bubblesort = ctk.CTkButton(
            self.left_side_panel, text="Bubble sort", command=lambda: self.show_frame(BubblesortPage))
        self.bt_bubblesort.grid(row=3, column=0, padx=(40, 20), pady=10)

        self.bt_quicksort = ctk.CTkButton(
            self.left_side_panel, text="Quick sort", command=lambda: self.show_frame(QuickSortPage))
        self.bt_quicksort.grid(row=4, column=0, padx=(40, 20), pady=10)

        self.bt_bigo = ctk.CTkButton(
            self.left_side_panel, text="BigO page", command=lambda: self.show_frame(BigOPage))
        ...
        self.right_side_panel = ctk.CTkFrame(
            self.main_container, corner_radius=10, fg_color="gray85")
        ...
        for F in (HomePage, IntroPage, BubblesortPage, QuickSortPage, BigOPage):
            ...
            frame.grid(in_=self.right_side_panel,
    ...
    def show_frame_by_number(self, i):
        self.show_frame((HomePage, IntroPage, BubblesortPage,
                        QuickSortPage, BigOPage)[i])
